...
WORKDIR /src
COPY ["InterviewDemo.csproj", "InterviewDemo/"]
RUN dotnet restore "InterviewDemo/InterviewDemo.csproj"

WORKDIR "/src/InterviewDemo"
COPY . .
RUN dotnet build "InterviewDemo.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "InterviewDemo.csproj" \
...
Execute unit tests
```
cd "InterviewDemo/test/InterviewDemo.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "InterviewDemo/src/InterviewDemo"
